...
class LowLevelEnvironment(Env):
    ...
    def __init__(self, airport_map):
        ...
        # Set probability that the robot will go in the intended direction if the
        # robot does not slip
    ...
    def next_state_and_reward_distribution(self, s, a, print_cell= False):
        ...
        # All other cases are moving in a given direction but with the possibility of slip.
        
        # First handle the slip case; here NONE is called which means we
        # sit in the same place, but we get the negative reward
            ...
            if (new_x < 0) or (new_x >= self._airport_map.width()) \
                ...
                if new_cell.is_obstruction():
                    ...
                    if new_cell.cell_type() is MapCellType.BAGGAGE_CLAIM:
                        r.append(-10)
                    elif new_cell.cell_type() is MapCellType.CHAIR:
                        r.append(-10)
                    else:
                        r.append(-1)
                    if print_cell:
